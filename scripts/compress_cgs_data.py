...
def parse_commented_bytes(file: io.TextIOBase, line):
    ...
    while len(line) != 0:
        ...
        if striped_line.startswith('COMMENTED_BYTES'):
            ...
            comment_index = striped_line.find('//')
            if comment_index >= 0:
                striped_line = striped_line[0:comment_index]
...
def compress_entry(option, entry):
    ...
    for item in entry.items:
        ...
        entry.items.insert(entry.items.index(item), Item(
            item.line, item.type, commented_bytes))
...
def export_entry(file: io.TextIOBase, entry):
    for item in entry.items:
        if item.type == 'entry':
            ...
            export_commented_bytes(
                file, item.nested_data, entry.name == 'Compressed data')
    ...
    for item in commented_bytes.items:
        ...
        if is_compressed_data and item.type == 'hex':
            ...
            for i in range(0, len(commented_bytes.data), 16):
                file.write(
                    indent + commented_bytes.data[i:i + 16].hex(' ') + '\n')
