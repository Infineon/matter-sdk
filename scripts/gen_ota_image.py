...
def main():
    ...
    parser.add_argument("--vendor_id", required=True, type=any_base_int)
    parser.add_argument("--product_id", required=True, type=any_base_int)
...
def run_ota_image_tool(option, configs):
    subprocess.run(
        [
            ...
            "create",
            "--vendor-id={}".format(option.vendor_id),
            "--product-id={}".format(option.product_id),
            "--version={}".format(
